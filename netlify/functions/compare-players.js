...
exports.handler = async (event, context) => {
  ...
  try {
    const { player1Email, player2Email } = JSON.parse(event.body);
    
    if (!player1Email || !player2Email) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'Email dei giocatori richieste' })
      };
    }
    ...
    if (!player1Data || !player2Data) {
      return {
        statusCode: 404,
        headers,
        body: JSON.stringify({ error: 'Uno o entrambi i giocatori non trovati' })
      };
    }
