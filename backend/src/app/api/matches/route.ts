...
async function updatePlayerStats(matchRecord: any, playerStats: any, scoreA: number, scoreB: number) {
  ...
  for (const playerEmail of allPlayers) {
    try {
      ...
      const partiteGiocate = (Number(currentStats.partiteGiocate) || 0) + 1;
      let partiteVinte = Number(currentStats.partiteVinte) || 0;
      let partitePerse = Number(currentStats.partitePerse) || 0;
      let partitePareggiate = Number(currentStats.partitePareggiate) || 0;
      ...
      const golFatti = (Number(currentStats.golFatti) || 0) + (playerMatchStats.gol || 0);
      const assistFatti = (Number(currentStats.assistFatti) || 0) + (playerMatchStats.assist || 0);
      const cartelliGialli = (Number(currentStats.cartelliGialli) || 0) + (playerMatchStats.gialli || 0);
      const cartelliRossi = (Number(currentStats.cartelliRossi) || 0) + (playerMatchStats.rossi || 0);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (completed && playerStats && scoreA !== undefined && scoreB !== undefined) {
      console.log('=== AGGIORNAMENTO STATISTICHE GIOCATORI ===');
      try {
        await updatePlayerStats(updatedRecord, playerStats, scoreA, scoreB);
